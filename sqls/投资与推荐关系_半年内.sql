-- psql  -f 投资与推荐关系_半年内.sql "dbname=xueshandai user=xsd password=Xsd123$"

-- 投资基本信息
drop table if exists task2_member_invest_base;
create table task2_member_invest_base as 
...
-- 用户投资信息
drop table if exists task2_member_invest;
create table task2_member_invest as 
select a.*
...
from task2_member_invest_base
group by investor_id
) a 
left outer join (select * from task2_member_invest_base where invest_order=1) b on a.investor_id = b.investor_id
left outer join (select * from task2_member_invest_base where invest_order=2) c on a.investor_id = c.investor_id
;



-- 推荐投资金额（推荐人限制在半年内）
drop table if exists task2_member_recommend_invest;
create table task2_member_recommend_invest as 
...
from (select * from recommend where date_created>='2017-12-01') a
left outer join task2_member_invest b on a.member_id=b.investor_id
group by referrer_id
;

-- 用户宽表
drop table if exists task2_member_wide;
create table task2_member_wide as
...
left outer join task2_member_invest i on a.member_id = i.investor_id
left outer join task2_member_recommend_invest ri on a.member_id = ri.recommender_id 
; 
