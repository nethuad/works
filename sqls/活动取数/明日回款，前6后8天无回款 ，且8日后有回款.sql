...
and need_receipt_time>=convert(varchar(10),DATEADD(dd,1,GETDATE()),120) and need_receipt_time<convert(varchar(10),DATEADD(dd,2,GETDATE()),120)
and capital>0
) a 
left outer join (
  ...
  where status in (1,4) and capital>0
   and ((need_receipt_time>=convert(varchar(10),DATEADD(dd,-6,GETDATE()),120) and need_receipt_time<convert(varchar(10),DATEADD(dd,1,GETDATE()),120)) 
    or (need_receipt_time>=convert(varchar(10),DATEADD(dd,2,GETDATE()),120) and need_receipt_time<convert(varchar(10),DATEADD(dd,9,GETDATE()),120)) )
) b on a.investor_id=b.investor_id
inner join (
  select distinct investor_id 
  from receipt_detail 
  where status in (1,4) and capital>0
   and need_receipt_time>=convert(varchar(10),DATEADD(dd,9,GETDATE()),120)
) c on a.investor_id=c.investor_id
